var board, // chess board obj
  ...
  promotionEvent,
  togglePlayer = false,
  engineDisabled = window.location.pathname === "/bot" ? false : true,
...
stockfish.onmessage = function (event) {
  ...
  if (stateHint == "grep") {
    ...
    if (match) {
      ...
      $("#board_chess_square_" + calcFieldNum(match[1])).css(
        "background",
        "#f7c5cd"
      );
      $("#board_chess_square_" + calcFieldNum(match[2])).css(
        "background",
        "#f7c5cd"
      );
  ...
  if (stateAnalyze == "grep") {
    var regex = new RegExp(
      "info depth " + staticSkill + " seldepth .*? pv (.*)"
    );
