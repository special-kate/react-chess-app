...
function InitImages() {
  ...
  for (ii = 0; ii < 2; ii++) {
    ...
    PiecePic[ii][0].src = ImagePath + ColorName[ii] + "k" + PieceFileExt;
    PiecePic[ii][1] = new Image();
    PiecePic[ii][1].src = ImagePath + ColorName[ii] + "q" + PieceFileExt;
    PiecePic[ii][2] = new Image();
    PiecePic[ii][2].src = ImagePath + ColorName[ii] + "r" + PieceFileExt;
    PiecePic[ii][3] = new Image();
    PiecePic[ii][3].src = ImagePath + ColorName[ii] + "b" + PieceFileExt;
    PiecePic[ii][4] = new Image();
    PiecePic[ii][4].src = ImagePath + ColorName[ii] + "n" + PieceFileExt;
    PiecePic[ii][5] = new Image();
    PiecePic[ii][5].src = ImagePath + ColorName[ii] + "p" + PieceFileExt;
  }
...
function OpenUrl(ss) {
  if (ss != "") parent.frames[1].location.href = ss;
    ...
    if (document.BoardForm.Url.value != "") {
      ...
      if (
        (nn || document.BoardForm.Url.value.indexOf(".htm") > 0) &&
        !document.layers
      ) {
        ...
        parent.frames[1].location.href =
          "pgnframe.html?" + document.BoardForm.Url.value;
...
function Init(rr) {
  ...
  if (FenString == StandardFen) {
    ...
    if (ll == FenString.length) {
      alert(
        "Invalid FEN [13]: char " + ll + " missing en passant target square"
      );
      Init("standard");
      return;
    }
...
function MoveBack(nn) {
  ...
  for (jj = 0; jj < nn && MoveCount > StartMove; jj++) {
    ...
    if (0 <= ii && ii < 16) {
      ...
      Board[HistPosX[1][cc]][HistPosY[1][cc]] =
        (HistType[1][cc] + 1) * (1 - 2 * MoveType);
    }
    ii = HistPiece[0][cc];
    Board[PiecePosX[MoveType][ii]][PiecePosY[MoveType][ii]] = 0;
    Board[HistPosX[0][cc]][HistPosY[0][cc]] =
      (HistType[0][cc] + 1) * (1 - 2 * MoveType);
    ...
    if (CurVar != 0) {
      if (MoveCount == ShortPgnMoveText[2][CurVar]) {
        ...
        if (
          !isCalculating &&
          document.BoardForm &&
          document.BoardForm.PgnMoveText
        )
  ...
  if (LastStyle)
    NewCommands[NewCommands.length] =
      LastStyle.charAt(0) + "last" + LastStyle.substr(1);
  if (ThisStyle)
    NewCommands[NewCommands.length] =
      ThisStyle.charAt(0) + "this" + ThisStyle.substr(1);
...
function MoveForward(nn, rr) {
            ...
            if (ffst >= 0) {
              ...
              if (mmove0 != "") {
                if (ParseMove(mmove0, true) > 0) {
                  ...
                  HistMove[MoveCount - StartMove] =
                    Math.floor((MoveCount + 2) / 2) + ". ... " + mmove1;
                  HistCommand[MoveCount - StartMove + 1] =
                    NewCommands.join("|");
                  MoveCount++;
                  MoveType = 1 - MoveType;
                } else {
  ...
  if (document.BoardForm) {
    ...
    if (nAudio && mmove1 != "")
      PlayAudio(HistMove[MoveCount - StartMove - 1], Delay / 12 + 200);
    if (
      mmove1 != "" &&
      isDragDrop &&
      nn == 1 &&
  ...
  if (AutoPlayInterval) clearTimeout(AutoPlayInterval);
  if (isAutoPlay)
    AutoPlayInterval = setTimeout("MoveForward(" + nn + ")", Delay);
}
...
function ParseMove(mm, sstore) {
  ...
  if (MoveCount > StartMove) {
    ...
    if (
      ...
      Math.abs(
        HistPosY[0][MoveCount - StartMove - 1] - PiecePosY[1 - MoveType][ii]
      ) == 2
    )
...
function EvalMove(ii, ttype0, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore) {
  ...
  if (ttype0 == 8) {
    ...
    if (
      StoreMove(
        0,
        0,
        PiecePosX[MoveType][0],
        PiecePosY[MoveType][0],
        -1,
        -1,
        -1,
        -1,
  ...
  if (ttype0 == 1) {
    if (
      Math.abs(PiecePosX[MoveType][ii] - xx1) !=
        Math.abs(PiecePosY[MoveType][ii] - yy1) &&
...
function StoreMove(ii, ttype1, xx1, yy1, jj, ttype3, xx3, yy3, sstore) {
  ...
  if (iis_check == 0 && sstore) {
    ...
    if (HistType[0][cc] != PieceType[MoveType][ii]) {
      if (MoveType == 0)
        MoveArray[cc] += PieceName.charAt(PieceType[MoveType][ii]);
      else
        MoveArray[cc] += PieceName.charAt(
          PieceType[MoveType][ii]
        ).toLowerCase();
    }
...
function IsCheck(xx, yy, tt) {
  ...
  if (
    Math.abs(PiecePosX[1 - tt][0] - xx) < 2 &&
    Math.abs(PiecePosY[1 - tt][0] - yy) < 2
  )
    return 1;
...
function ExecCommands(nnc, hh) {
  ...
  for (ii = 0; ii < OldCommands.length; ii++) {
    ...
    if (tt == "B" || tt == "C") {
      nn =
        OldCommands[ii].charCodeAt(1) -
        97 +
        (8 - parseInt(OldCommands[ii].charAt(2))) * 8;
      if (isRotated) nn = 63 - nn;
      if (nn >= 0 && nn <= 63) {
        if (tt == "B")
          document.images[ImageOffset + nn].style.borderColor = BorderColor;
        else
          document.images[ImageOffset + nn].style.backgroundColor =
            "transparent";
      }
    }
    if (tt == "A" || tt == "B")
    ...
    if (tt == "this" || tt == "last") {
      ...
      if (kk >= 0) {
        ...
        if (tt == "B" || tt == "C") {
          NewCommands[nn] =
            tt +
            String.fromCharCode(97 + HistPosX[0][kk]) +
            (1 + HistPosY[0][kk]) +
            cc;
          NewCommands[nn + 1] =
            tt +
      ...
      if (tt == "B" || tt == "C") {
        ...
        if (nn >= 0 && nn <= 63) {
          ...
          if (tt == "B")
            document.images[ImageOffset + nn].style.borderColor = cc.substr(
              0,
              7
            );
          else
      ...
      if (tt == "A" && dd > 0) {
        ...
        if (nn >= 0 && nn <= 63 && mm >= 0 && mm <= 63) {
          ...
          if (NewCommands[ii].charAt(4) == "0")
            aa += GetArrow(xx0, yy0, 0, 0, HEX2RGBA(cc), dd);
      ...
      for (ii = 0; ii < 8; ii++) {
        for (jj = 0; jj < 8; jj++) {
          if (Board[ii][jj] == 0) SetImg(63 - ii - (7 - jj) * 8, BoardPic);
            ...
            SetImg(
              63 - ii - (7 - jj) * 8,
              PiecePic[(1 - sign(Board[ii][jj])) / 2][
                Math.abs(Board[ii][jj]) - 1
              ]
            );
        }
      }
...
function SetBCStyle(ss) {
  ...
  if (!ss) {
    BCCmds = "";
    if (LastStyle) BCCmds += LastStyle.charAt(0) + "last" + LastStyle.substr(1);
    if (ThisStyle) {
      if (LastStyle) BCCmds += ",";
...
function HighlightCandidates(nn, ccs) {
  ...
  if (nna > 0 && ccs != " ") {
    ...
    if (tt == "A") {
      ...
      for (jj = 1; jj < nna; jj++)
        bb +=
          "," + tt + String.fromCharCode(ii0 + 97) + (jj0 + 1) + aa[jj] + cc;
    } else {
      bb = tt + aa[0] + cc;
...
function AddBCCmd(nn, bb) {
  ...
  if (BCStyle.charAt(0) == "A") {
    ...
    if (ll >= 0) BCCmds = BCCmds.replace("i0", mm);
      ...
      if (BCCmds) BCCmds += ",";
      BCCmds += "A" + mm + "i0" + BCStyle.substr(1);
    }
  } else {
    if (BCCmds) BCCmds += ",";
...
function SetBoardClicked(nn) {
  ...
  if (BoardClicked >= 0) {
    if (BoardClicked < 64) {
      if (isRotated)
        document.images[ImageOffset + 63 - BoardClicked].style.borderColor =
          BorderColor;
      else
        document.images[ImageOffset + BoardClicked].style.borderColor =
          BorderColor;
    } else
      document.images[ImageOffset + BoardClicked + 3].style.borderColor =
        BorderColor;
  }
  BoardClicked = nn;
  if (BoardClicked >= 0) {
...
function BoardClick(nn, bb) {
  ...
  if (BoardClicked < 0) {
    ...
    if (sign(Board[ii0][jj0]) != ((MoveCount + 1) % 2) * 2 - 1) {
      mm = "---";
      if (document.BoardForm && document.BoardForm.PgnMoveText)
        ShortPgnMoveText[0][CurVar] = Uncomment(
          document.BoardForm.PgnMoveText.value
        );
      ssearch = Math.floor(MoveCount / 2 + 1) + ".";
      ...
      if (vv < ShortPgnMoveText[0].length + 1) {
        if (
          RecordCount == 0 &&
          !(document.BoardForm && document.BoardForm.PgnMoveText)
        ) {
        ...
        if (RecordCount == 0) RecordedMoves = HistMove[MoveCount - StartMove];
          ...
          if (MoveType == 0)
            RecordedMoves += " " + HistMove[MoveCount - StartMove];
  ...
  if (Math.abs(Board[ii0][jj0]) != 1) {
    ...
    if (Math.abs(Board[ii0][jj0]) == 6) {
      if (mm.charAt(1) == "x")
        mmm = mm.substr(0, 1) + eval(jj0 + 1) + mm.substr(1, 11);
  ...
  if (
    IsCheck(
      PiecePosX[1 - MoveType][0],
      PiecePosY[1 - MoveType][0],
      1 - MoveType
    )
  )
    mm += "+";
...
function ShowSAN(ss) {
  ...
  if (
    ShowPieceName == "" ||
    ShowPieceName == PieceName ||
    ShowPieceName == "Figure"
  )
    return;
  if (document.BoardForm && document.BoardForm.PgnMoveText) {
    ...
    ww.document.open();
    ww.document.writeln(
      "<HTML><HEAD></HEAD><BODY>" + TransformSAN(tt) + "</BODY></HTML>"
    );
    ww.document.close();
  }
}
...
function ShowFENList() {
  ...
  ww.document.open();
  ww.document.writeln(
    "<HTML><HEAD></HEAD><BODY><PRE>" + GetFENList() + "</PRE></BODY></HTML>"
  );
  ww.document.close();
}
...
function MakePuzzle() {
  ...
  if (ShowPieceName != "" && ShowPieceName != PieceName)
    aa += "&ShowSAN=" + ShowPieceName;
  if (tt != "") tt = "&AddText=" + tt;
  ...
  if (MoveCount > 2) {
    ff =
      ff_old[0] + " " + ff_old[1] + " " + ff_old[2] + " " + ff_old[3] + " 0 1";
    ii = Math.floor(MoveCount / 2 + 1);
    nn = ii + 1;
    while (mm.indexOf(nn + ".") >= 0) nn++;
    while (nn > ii) {
      nn--;
      while (mm.indexOf(nn + ".") >= 0)
        mm = mm.replace(nn + ".", nn - ii + 1 + ".");
    }
...
function MakeGamelink() {
  ...
  if (ShowPieceName != "" && ShowPieceName != PieceName)
    aa += "&ShowSAN=" + ShowPieceName;
  mm =
    "&eval=AddText(unescape(%22" +
    escape("<|td><td>" + tt) +
    "%22)+GetHTMLMoveText(0,0,1))";
...
function GetHTMLMoveText(vvariant, nnoscript, ccommenttype, sscoresheet) {
  ...
  if (vvariant) {
    ...
    if (!isNaN(ShortPgnMoveText[0][vv])) {
      ...
      if (MoveCount != ShortPgnMoveText[0][vv])
        return "(" + ShortPgnMoveText[0][vv] + ")";
  ...
  for (ii = jj; ii < ll && activeAnchor < 0; ii++) {
    if (TargetDocument.anchors[ii].name == nn) {
      ...
      if (
        ...
        TargetDocument.anchors[offsetAnchor + activeAnchor].scrollIntoView
      ) {
...
function UpdateAnnotation(bb) {
  ...
  if (bb) {
    with (parent.frames["annotation"].document) {
      ...
      writeln(
        "<input type='hidden' name='MoveCount' value='" + MoveCount + "'>"
      );
...
function GetCapturedPieces(iip, bb) {
  ...
  for (ll = 0; ll > pp0[0]; ll--) {
    rr[kk % 4] +=
      "<td><IMG SRC='" + iip + ColorName[ii] + tt[5] + PieceFileExt + "'></td>";
    kk++;
  }
  while (kk < 4) {
...
function GetBorder(theX0, theY0, theColor, theBoardSize) {
  //var bborder=2;
  var ssize = Math.floor(theBoardSize / 8),
    bborder = 1 + Math.floor(theBoardSize / 128);
  dd =
    "<div style='visibility:hidden;position:absolute;left:" +
    eval(theX0 * ssize) +
    "px;top:" +
    eval(theY0 * ssize) +
    "px;width:" +
    ...
    "px;z-index:99'>";
  dd +=
    "<div style='visibility:visible;position:absolute;left:0px;top:0px;width:" +
    eval(ssize - bborder) +
    "px;height:" +
    bborder +
    "px;background-color:" +
  ...
  dd +=
    ...
    ";font-size:1pt;line-height:1pt;'></div>";
  dd +=
    "<div style='visibility:visible;position:absolute;left:0px;top:" +
    bborder +
    "px;width:" +
    bborder +
    "px;height:" +
...
function GetArrow(theX0, theY0, theX1, theY1, theColor, theBoardSize) {
  var ssize = 1 + Math.floor(theBoardSize / 80),
    ssize2 = Math.floor(ssize / 2);
  //var ccL=9*ssize2, ccB=3*ssize2;
  var ccL = Math.floor(theBoardSize / 17),
    ccB = Math.floor(theBoardSize / 51);
  var xx0 = theX0,
    yy0 = theY0,
    xx1 = theX1,
    yy1 = theY1;
  ...
  var ss =
    "<div style='visibility:hidden;position:absolute;left:" +
    Math.round((xx0 + xx1 - nn) / 2) +
    "px; top:" +
    Math.round((yy0 + yy1 - nn) / 2) +
    "px; width:" +
    Math.round(nn) +
    "px; height:" +
    Math.round(nn) +
    ...
    aa +
    "deg); transform-origin: 50% 50%; z-index:100'>";
  ss +=
    "<div style='visibility:visible;position:absolute;left:" +
    Math.floor(nn / 2 - ccB) +
    "px;top:0px,width:0px;height:0px;border-style: solid; border-width: 0px " +
    ccB +
    "px " +
    ccL +
    "px " +
  ...
  ss +=
    "<div style='visibility:visible;position:absolute;left:" +
    Math.floor(nn / 2 - ssize2) +
    "px;top:" +
    ccL +
    "px;width:0px;height:0px;border-style: solid; border-width: " +
    Math.ceil(nn - ccL) +
    "px 0px 0px " +
    ssize +
    "px;border-color: " +
...
function GetOldArrow(theX0, theY0, theX1, theY1, theColor, theBoardSize) {
  ...
  //var ssize=2;
  var ssize = 1 + Math.floor(theBoardSize / 128),
    ssize2 = Math.floor(ssize / 2),
    bborder = 8 * ssize;
  var ddir =
    (theY1 > theY0 && theX1 > theX0) || (theY1 < theY0 && theX1 < theX0)
      ? true
      : false;
  if (theX0 <= theX1) {
    ll = theX0;
  ...
  dd =
    "<div style='visibility:hidden;position:absolute;left:" +
    eval(ll - ssize2 - bborder) +
    "px;top:" +
    eval(tt - ssize2 - bborder) +
    "px;width:" +
    eval(ww + ssize + 2 * bborder) +
    "px;height:" +
    eval(hh + ssize + 2 * bborder) +
    "px;z-index:100'>";
  if (ww == 0 || hh == 0)
    dd +=
      "<div style='visibility:visible;position:absolute;left:" +
  ...
  var ccL = Math.round(theBoardSize / 28),
    ccB = Math.round(theBoardSize / 67);
  var DDX = theX1 - theX0,
    DDY = theY1 - theY0;
  ...
  ddir =
    (theY1 > tmpY0 && theX1 > tmpX0) || (theY1 < tmpY0 && theX1 < tmpX0)
      ? true
      : false;
  if (tmpX0 <= theX1) {
    ll = tmpX0;
    ...
    if (ww > hh) {
      ...
      while (ccr < ww) {
        ...
        if (ddir)
          dd +=
            "<div style='visibility:visible;position:absolute;left:" +
            eval(ccl + bborder + ll - ll0) +
            "px;top:" +
            eval(cct + bborder + tt - tt0) +
            "px;width:" +
            eval(ccr - ccl + ssize) +
            "px;height:" +
            ssize +
...
function ParsePgn(nn, gg, ffile) {
  ...
  if (ffile) ss = " " + ffile;
    ...
    if (!parent.frames[1].document.documentElement) {
      if (nn > -50)
        setTimeout("ParsePgn(" + eval(nn - 5) + ",'" + gg + "')", 400);
      return;
    }
  ...
  for (ii = 0; ii < tt.length; ii++) {
    ...
    if (kk < tt[ii][1].length) {
      ...
      while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
        ppgnText[jj] =
          ppgnText[jj].substr(0, kk) + "" + ppgnText[jj].substr(kk + 1);
      kk = 0;
      ff = String.fromCharCode(10) + String.fromCharCode(10);
      while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
        ppgnText[jj] =
          ppgnText[jj].substr(0, kk) +
          " <br /><br /> " +
          ppgnText[jj].substr(kk + 2);
      kk = 0;
      ff = String.fromCharCode(10);
      while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
        ppgnText[jj] =
          ppgnText[jj].substr(0, kk) + " " + ppgnText[jj].substr(kk + 1);
      while (ffenText[jj].indexOf('"') >= 0)
        ffenText[jj] = ffenText[jj].replace('"', "");
      while (ssanText[jj].indexOf('"') >= 0)
        ssanText[jj] = ssanText[jj].replace('"', "");
      if (ParseType % 2 == 1) {
  ...
  if (ssh || ParseType > 2) {
    dd.writeln(
      "table { border-left:1px solid #808080; border-top:1px solid #808080; }"
    );
    dd.writeln(
  ...
  if (document.BoardForm && document.BoardForm.ImagePath)
    ii =
      document.BoardForm.ImagePath.options[
        document.BoardForm.ImagePath.options.selectedIndex
      ].value;
  ...
  if (parent.frames["annotation"])
    ...
    dd.writeln(
      "if (! parent.frames[0]) location.href='ltpgnviewer.html?'+location.href+'" +
        ii +
        "';"
    );
  ...
  for (ii = 0; ii < jj; ii++) {
    ...
    if (ffenText[ii] != "")
      dd.writeln("FenText[" + ii + "]='" + ffenText[ii] + "';");
    if (ssanText[ii] != "")
      dd.writeln("SanText[" + ii + "]='" + ssanText[ii] + "';");
  }
  ...
  dd.writeln("    { if (nn>=0)");
  dd.writeln(
    "      { if (FenText[nn]) parent.frames[0].Init(" + uu + "(FenText[nn]));"
  );
  dd.writeln("        else parent.frames[0].Init('standard');");
  dd.writeln(
    "        if (SanText[nn]) parent.frames[0].ApplySAN(" +
      uu +
      "(SanText[nn]));"
  );
  dd.writeln(
  ...
  if (isDragDrop)
    dd.writeln(
      "      if (parent.frames[0].SetDragDrop) parent.frames[0].SetDragDrop(1);"
    );
  dd.writeln("      return;");
  dd.writeln("    }");
  dd.writeln("  }");
  ...
  if (jj > 1) {
    ...
    dd.writeln("  { for (ii=1; ii<kk; ii++)");
    dd.writeln(
      "    { if (ll.options[(ii+jj)%kk+1].text.toLowerCase().indexOf(tt)>=0)"
    );
    dd.writeln("      { ll.selectedIndex=(ii+jj)%kk+1;");
    dd.writeln("        OpenGame(ll.options[(ii+jj)%kk+1].value);");
    dd.writeln("        return(false);");
    ...
    dd.writeln("  for (ii=1; ii<kk; ii++)");
    dd.writeln(
      "  { var nn, mm=oo.length+3, ss=GameText[(ii+jj)%kk].split('<br />');"
    );
    dd.writeln("    for (nn=0; nn<ss.length; nn++)");
    dd.writeln(
      "    { if ((ss[nn].indexOf(oo)>0)&&(ss[nn].toLowerCase().indexOf(tt)>=mm))"
    );
    dd.writeln("      { ll.selectedIndex=(ii+jj)%kk+1;");
    dd.writeln("        OpenGame(ll.options[(ii+jj)%kk+1].value);");
    dd.writeln("        return(false);");
  ...
  dd.writeln("</" + "script>");
  if (jj == 1)
    dd.writeln("</head><body onLoad=\"setTimeout('OpenGame(0)',400)\">");
  else {
    if (ParseType < 3) {
      if (parseInt(gg) < jj)
        dd.writeln(
          "</head><body onLoad=\"setTimeout('OpenGame(" + gg + ")',400)\">"
        );
      ...
      dd.writeln(
        "<option>Moves</option></select><INPUT type='submit' value='search'>"
      );
      if (jj < 24) dd.writeln("//-->");
...
function ScoreSheetHeader(tt) {
  ...
  if (vv[10])
    ss +=
      "<tr><td colspan=14><small>fen</small><br /><nobr>" +
      vv[10] +
      "</nobr></td></tr>";
  ss += "</table>";
  return ss;
...
function PrintPosition() {
  ...
  with (ww.document) {
    open();
    writeln(
      "<html><head><title>" + tt + "</title></head><body><div align='center'>"
    );
...
function AnimateBoard(nn) {
  ...
  if (nn % mm == 1) {
    if (isRotated)
      dragImg[pp % 3] =
        document.images[
          63 - dragPiece[pp + 2] - (7 - dragPiece[pp + 3]) * 8 + ImageOffset
        ];
    else
      dragImg[pp % 3] =
        document.images[
          dragPiece[pp + 2] + (7 - dragPiece[pp + 3]) * 8 + ImageOffset
        ];
    ...
    if (isRotated)
      dragImg[pp % 3] =
        document.images[
          63 - dragPiece[pp + 0] - (7 - dragPiece[pp + 1]) * 8 + ImageOffset
        ];
    else
      dragImg[pp % 3] =
        document.images[
          dragPiece[pp + 0] + (7 - dragPiece[pp + 1]) * 8 + ImageOffset
        ];
  ...
  if (nn % mm != 0) {
    ...
    dragImg[pp % 3].style.left =
      Math.round(
        ((nn % mm) * (dragPiece[pp + 2] - dragPiece[pp + 0])) / (mm - 1)
      ) +
      dragImgBorder +
      "px";
    dragImg[pp % 3].style.top =
      Math.round(
        ((nn % mm) * (dragPiece[pp + 3] - dragPiece[pp + 1])) / (mm - 1)
      ) +
      dragImgBorder +
      "px";
    if (dragPiece[4] >= 0 && mm - 1 == nn)
      setTimeout("AnimateBoard(" + (mm + 1) + ")", 50);
